...
//                ->add('username', TextType::class, array('required' => TRUE, 'attr' => array('required' => TRUE, 'placeholder' => "Nom d'utilisateur *", 'class' => 'form-control')))
                ->add('tel1', TextType::class, array('required' => TRUE, 'attr' => array('required' => TRUE, 'placeholder' => "Téléphone *", 'maxlength' => $this->maxLengthTel, 'class' => 'form-control numeric')))
        ...
        if ($this->idProfil == 0) {
            ...
            $builder->add('profil', EntityType::class, 
             array(
                'class' => $this->userBundle . 'Profil',
                'choice_label' => 'nom',
                'multiple' => false,
                ...
                'attr' => array('required' => TRUE, 'class' => 'form-control')
                    ...
                    $builder->add('profil', 'entity', array(
                        'class' => $this->userBundle . 'Profil',
                        'property' => 'nom',
                        'em'=>"pgsql",
                        'multiple' => false,
                        'query_builder' => function (\Doctrine\ORM\EntityRepository $repository) {
                            $qb = $repository->createQueryBuilder('m')
                                    ->where('m.id =:idP')
                                    ->setParameters(array('idP' => $this->idProfil));
                            return $qb;
                        },
                                'required' => TRUE,
                                'attr' => array('required' => TRUE, 'class' => 'form-control')));
